...
export default function Connexion({
  ...
  function getClient() {
    fetch(`http://localhost:5000/client/${email}/${mot_de_passe}`)
      .then(res => res.json())
      .then(
        (result) => {
          let data = result
  ...
  return (
    ...
    >
      ...
      >
        ...
        style={{background: nameUser!==null&& Object.keys(nameUser).length>0 ? "green"  :"black"}}>
          {/*condition pour pour vérifier est-ce-que dans keys il y'a des données ou non */}
          {nameUser!==null&& Object.keys(nameUser).length>0  ? "Compte connecté" : "Connexion"}
        </button>
        <Modal isOpen={modal} cnx={cnx}>
          {!inscrire ? 
          nameUser!==null&& Object.keys(nameUser).length>0  ? <>
          <ModalHeader cnx={cnx}>Compte connecté <span style={{ marginLeft: "277px", cursor: "pointer" }}
            onClick={cnx}><CloseIcon /></span></ModalHeader>
          <ModalBody>
            <FormGroup row>
          ...
          <ModalFooter>
            ...
            >
           {nameUser!==null&& Object.keys(nameUser).length>0 ? "Se déconnecter" : "Se connecter"}   
            </button>
          </ModalFooter>
        </>:
            <>
              <ModalHeader cnx={cnx}>Connexion <span style={{ marginLeft: "335px", cursor: "pointer" }}
                onClick={cnx}><CloseIcon /></span></ModalHeader>
              <ModalBody>
                <FormGroup row>
                  <Label for="exampleEmail" sm={2}>
          ...
          : (
            <>
              ...
              <ModalHeader >Inscription <span style={{ marginLeft: "335px", cursor: "pointer" }}
                onClick={cnx}><CloseIcon /></span></ModalHeader>
              <ModalBody>
                <FormGroup row>
                  <Label for="exampleEmail" sm={2}>
